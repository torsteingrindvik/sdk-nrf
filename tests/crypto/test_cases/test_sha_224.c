...
LOG_MODULE_REGISTER(test_sha_224, LOG_LEVEL_INF);

extern test_vector_hash_t __start_test_vector_hash_224_data[];
extern test_vector_hash_t __stop_test_vector_hash_224_data[];

extern test_vector_hash_t __start_test_vector_hash_224_long_data[];
extern test_vector_hash_t __stop_test_vector_hash_224_long_data[];

#define INPUT_BUF_SIZE (4125)
#define OUTPUT_BUF_SIZE (64)
#define USE_SHA_224 1

//Same context is used for SHA-224 and SHA-256
static mbedtls_sha256_context sha224_context;
...
void sha_224_clear_buffers(void)
{
...
__attribute__((noinline)) static void sha_224_setup(void)
{
	sha_224_clear_buffers();
	p_test_vector = ITEM_GET(test_vector_hash_224_data, test_vector_hash_t,
				 sha_vector_n);
	unhexify_sha();
}

static void sha_224_teardown(void)
{
	sha_vector_n++;
}

__attribute__((noinline)) static void sha_224_long_setup(void)
{
	sha_224_clear_buffers();
	p_test_vector = ITEM_GET(test_vector_hash_224_long_data,
				 test_vector_hash_t, sha_long_vector_n);
	unhexify_sha_long();
}

static void sha_224_long_teardown(void)
{
	sha_long_vector_n++;
}

/**@brief Function encapsulating SHA-224 execution steps.
 *
 */
static int exec_sha_224(test_vector_hash_t *p_test_vector, int in_len,
			bool is_long)
{
	mbedtls_sha256_init(&sha224_context);
	int err_code = mbedtls_sha256_starts_ret(&sha224_context, USE_SHA_224);
	TEST_VECTOR_ASSERT_EQUAL(0, err_code);

	/* Update the hash. */
	if (!is_long) {
		err_code = mbedtls_sha256_update_ret(&sha224_context,
		...
		for (int j = 0; j < p_test_vector->update_iterations; j++) {
			...
			err_code = mbedtls_sha256_update_ret(
				&sha224_context, m_sha_input_buf, in_len);
	...
	/* Finalize the hash. */
	return mbedtls_sha256_finish_ret(&sha224_context, m_sha_output_buf);
}

/**@brief Function for verifying the SHA digest of messages.
 */
void exec_test_case_sha_224(void)
{
	int err_code = -1;

	start_time_measurement();
	err_code = exec_sha_224(p_test_vector, in_len, false);
	...
	mbedtls_sha256_free(&sha224_context);
}

/**@brief Function for verifying the SHA digest of long messages.
 */
void exec_test_case_sha_224_long(void)
{
	int err_code = -1;

	start_time_measurement();
	err_code = exec_sha_224(p_test_vector, in_len, true);
	stop_time_measurement();
...
ITEM_REGISTER(test_case_sha_224_data, test_case_t test_sha_224) = {
	.p_test_case_name = "SHA-224",
	.setup = sha_224_setup,
	.exec = exec_test_case_sha_224,
	.teardown = sha_224_teardown,
	.vector_type = TV_HASH,
	.vectors_start = __start_test_vector_hash_224_data,
	.vectors_stop = __stop_test_vector_hash_224_data,
};

ITEM_REGISTER(test_case_sha_224_data, test_case_t test_sha_224_long) = {
	.p_test_case_name = "SHA-224 long",
	.setup = sha_224_long_setup,
	.exec = exec_test_case_sha_224_long,
	.teardown = sha_224_long_teardown,
	.vector_type = TV_HASH,
	.vectors_start = __start_test_vector_hash_224_long_data,
	.vectors_stop = __stop_test_vector_hash_224_long_data,
};
