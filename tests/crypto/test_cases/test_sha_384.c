...
LOG_MODULE_REGISTER(test_sha_384, LOG_LEVEL_INF);

extern test_vector_hash_t __start_test_vector_hash_384_data[];
extern test_vector_hash_t __stop_test_vector_hash_384_data[];

extern test_vector_hash_t __start_test_vector_hash_384_long_data[];
extern test_vector_hash_t __stop_test_vector_hash_384_long_data[];

#define INPUT_BUF_SIZE (4125)
#define OUTPUT_BUF_SIZE (64)
#define USE_SHA_384 1

static mbedtls_sha512_context sha384_context;
...
void sha_384_clear_buffers(void)
{
...
__attribute__((noinline)) static void sha_384_setup(void)
{
	sha_384_clear_buffers();
	p_test_vector = ITEM_GET(test_vector_hash_384_data, test_vector_hash_t,
				 sha_vector_n);
	unhexify_sha();
}

static void sha_384_teardown(void)
{
	sha_vector_n++;
}

__attribute__((noinline)) static void sha_384_long_setup(void)
{
	sha_384_clear_buffers();
...
static void sha_384_long_teardown(void)
{
	sha_long_vector_n++;
}

/**@brief Function encapsulating SHA-384 execution steps.
 *
 */
static int exec_sha_384(test_vector_hash_t *p_test_vector, int in_len,
			bool is_long)
{
	mbedtls_sha512_init(&sha384_context);
	int err_code = mbedtls_sha512_starts_ret(&sha384_context, USE_SHA_384);
	TEST_VECTOR_ASSERT_EQUAL(0, err_code);

	/* Update the hash. */
	if (!is_long) {
		err_code = mbedtls_sha512_update_ret(&sha384_context,
		...
		for (int j = 0; j < p_test_vector->update_iterations; j++) {
			...
			err_code = mbedtls_sha512_update_ret(
				&sha384_context, m_sha_input_buf, in_len);
	...
	/* Finalize the hash. */
	return mbedtls_sha512_finish_ret(&sha384_context, m_sha_output_buf);
}

/**@brief Function for verifying the SHA digest of messages.
 */
void exec_test_case_sha_384(void)
{
	int err_code = -1;

	start_time_measurement();
	err_code = exec_sha_384(p_test_vector, in_len, false);
 ...
 */
void exec_test_case_sha_384_long(void)
{
	int err_code = -1;

	start_time_measurement();
	err_code = exec_sha_384(p_test_vector, in_len, true);
	stop_time_measurement();
...
ITEM_REGISTER(test_case_sha_384_data, test_case_t test_sha_384) = {
	.p_test_case_name = "SHA-384",
	.setup = sha_384_setup,
	.exec = exec_test_case_sha_384,
	.teardown = sha_384_teardown,
	.vector_type = TV_HASH,
	.vectors_start = __start_test_vector_hash_384_data,
	.vectors_stop = __stop_test_vector_hash_384_data,
};

ITEM_REGISTER(test_case_sha_384_data, test_case_t test_sha_384_long) = {
	.p_test_case_name = "SHA-384 long",
	.setup = sha_384_long_setup,
	.exec = exec_test_case_sha_384_long,
	.teardown = sha_384_long_teardown,
	.vector_type = TV_HASH,
	.vectors_start = __start_test_vector_hash_384_long_data,
	.vectors_stop = __stop_test_vector_hash_384_long_data,
};
