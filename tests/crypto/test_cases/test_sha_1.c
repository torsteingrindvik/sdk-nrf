...
LOG_MODULE_REGISTER(test_sha_1, LOG_LEVEL_INF);

extern test_vector_hash_t __start_test_vector_hash_sha_1_data[];
extern test_vector_hash_t __stop_test_vector_hash_sha_1_data[];

extern test_vector_hash_t __start_test_vector_hash_sha_1_long_data[];
extern test_vector_hash_t __stop_test_vector_hash_sha_1_long_data[];

#define INPUT_BUF_SIZE (4125)
#define OUTPUT_BUF_SIZE (64)

static mbedtls_sha1_context sha1_context;
...
__attribute__((noinline)) static void sha_1_setup(void)
{
	sha_1_clear_buffers();
	p_test_vector = ITEM_GET(test_vector_hash_sha_1_data, test_vector_hash_t,
				 sha_vector_n);
	unhexify_sha();
}

static void sha_1_teardown(void)
{
	sha_vector_n++;
}

__attribute__((noinline)) static void sha_1_long_setup(void)
{
	sha_1_clear_buffers();
	p_test_vector = ITEM_GET(test_vector_hash_sha_1_long_data,
				 test_vector_hash_t, sha_long_vector_n);
	unhexify_sha_long();
}

static void sha_1_long_teardown(void)
{
	sha_long_vector_n++;
}

/**@brief Function encapsulating SHA-1 execution steps.
 *
 */
static int exec_sha_1(test_vector_hash_t *p_test_vector, int in_len,
			bool is_long)
{
	mbedtls_sha1_init(&sha1_context);
	int err_code = mbedtls_sha1_starts_ret(&sha1_context);
	TEST_VECTOR_ASSERT_EQUAL(0, err_code);

	/* Update the hash. */
	if (!is_long) {
		err_code = mbedtls_sha1_update_ret(&sha1_context,
		...
		for (int j = 0; j < p_test_vector->update_iterations; j++) {
			...
			err_code = mbedtls_sha1_update_ret(
				&sha1_context, m_sha_input_buf, in_len);
	...
	/* Finalize the hash. */
	return mbedtls_sha1_finish_ret(&sha1_context, m_sha_output_buf);
}

/**@brief Function for verifying the SHA digest of messages.
 */
void exec_test_case_sha_1(void)
{
	int err_code = -1;

	start_time_measurement();
	err_code = exec_sha_1(p_test_vector, in_len, false);
	stop_time_measurement();

	/* Verify the mbedtls_sha1_finish_ret err_code. */
	TEST_VECTOR_ASSERT_EQUAL(p_test_vector->expected_err_code, err_code);
	...
	err_code = mbedtls_sha1_ret(m_sha_input_buf, in_len, m_sha_output_buf);
 ...
 */
void exec_test_case_sha_1_long(void)
{
	int err_code = -1;

	start_time_measurement();
	err_code = exec_sha_1(p_test_vector, in_len, true);
	stop_time_measurement();
...
ITEM_REGISTER(test_case_sha_1_data, test_case_t test_sha_1) = {
	.p_test_case_name = "SHA-1",
	.setup = sha_1_setup,
	.exec = exec_test_case_sha_1,
	.teardown = sha_1_teardown,
	.vector_type = TV_HASH,
	.vectors_start = __start_test_vector_hash_sha_1_data,
	.vectors_stop = __stop_test_vector_hash_sha_1_data,
};

ITEM_REGISTER(test_case_sha_1_data, test_case_t test_sha_1_long) = {
	.p_test_case_name = "SHA-1 long",
	.setup = sha_1_long_setup,
	.exec = exec_test_case_sha_1_long,
	.teardown = sha_1_long_teardown,
	.vector_type = TV_HASH,
	.vectors_start = __start_test_vector_hash_sha_1_long_data,
	.vectors_stop = __stop_test_vector_hash_sha_1_long_data,
};
